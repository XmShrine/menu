...
int menu_str(int x, int y, int l, char* str) {
    ...
    wid.ly = e;
    wid.content = str;
    widget_rcontent(wid, fmt);
    // array* temp = widget_content(wid, fmt);
    // array_drop(&temp);
    return 0;
}
...
int pmenu_init(struct pmenu pm) {
    ...
    if (pm.title.content && *pm.title.content) {
        ...
        if (pm.title.color) {
            rrcolor(pm.title.color);
            // array* temp = frcolor(pm.title.color);
            // array_append(scrstr, temp);
            // array_drop(&temp);
        }
        if (pm.title.box) {
            widget_rbox(wid_a, *pm.title.box);
            // array* tmp = widget_box(wid_a, *pm.title.box);
            // array_drop(&tmp);
        }
        if (pm.title.fmt) {
            fmt = *pm.title.fmt;
        }
        widget_rcontent(wid_b, fmt);
        // array* tmp = widget_content(wid_b, fmt);
        // array_drop(&tmp);
        e += 2;
        h += e;
    }
    ...
    if (pm.main.content && !array_is_empty(pm.main.content)) {
        ...
        h += wid_c.ly;
        if (pm.main.color) {
            rrcolor(pm.main.color);
            // array* temp = frcolor(pm.main.color);
            // array_append(scrstr, temp);
            // array_drop(&temp);
        }
        if (pm.main.box) {
            widget_rbox(wid_c, *pm.main.box);
            // array* tmp = widget_box(wid_c, *pm.main.box);
            // array_drop(&tmp);
        }
        if (pm.title.fmt) {
            fmt = *pm.title.fmt;
        ...
        for (size_t i=0; i<array_len(pm.main.content); i++) {
            ...
            widget_rcontent(wid_d, fmt);
            // array* tmp = widget_content(wid_d, fmt);
            // array_drop(&tmp);
            x+=dev;
        }
    }
